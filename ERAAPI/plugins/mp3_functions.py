...
BASE_HEADERS = {
    ...
    "Accept": "*/*",
    "Referer": "https://v3.mp3juices.click/",
}
...
async def youtube_to_mp3_flvto(video_id: str) -> Dict[str, Optional[str]]:
    try:
...
async def youtube_to_mp3_flvto_alt(video_id: str) -> Dict[str, Optional[str]]:
    try:
        async with aiohttp.ClientSession() as session:
            # Step 1: Search (flow maintain karne ke liye)
            search_url = f"https://test.flvto.online/search/?q=https://www.youtube.com/watch?v={video_id}"
...
def _flvto_cloudscraper_sync(video_id: str) -> Dict[str, Optional[str]]:
    ...
    try:
        ...
        headers = {
            **FLVTO_CF_HEADERS,
            "Referer": f"https://ht.flvto.online/widget?url=https://www.youtube.com/watch?v={video_id}&el=267",
        }
        payload = {"id": video_id, "fileType": "MP3"}
...
async def youtube_to_mp3_flvto_cloudscraper(video_id: str) -> Dict[str, Optional[str]]:
    """Async wrapper for cloudscraper method"""
    try:
        # Run sync cloudscraper in thread pool to make it async-compatible
        result = await asyncio.to_thread(_flvto_cloudscraper_sync, video_id)
        return result
    except Exception as e:
...
async def youtube_to_mp3_ytmp3(video_id: str) -> Dict[str, Optional[str]]:
    try:
        async with httpx.AsyncClient(timeout=20, headers=YT_MP3_HEADERS) as client:
            ...
            convert_url = f"{init_data['convertURL']}&v={video_id}&f=mp3&_={_ts()}"
...
async def youtube_to_mp3_ytmp3_alt(video_id: str) -> Dict[str, Optional[str]]:
    try:
        async with aiohttp.ClientSession() as session:
            ts = _ts()
            ...
            final_url = f"https://www0.ummn.nu/?id={video_id}&t={ts+5}"
...
async def youtube_to_mp3_insvid(video_id: str) -> Dict[str, Optional[str]]:
    try:
        async with httpx.AsyncClient(timeout=20, headers=BASE_HEADERS) as client:
            button_url = f"https://ac.insvid.com/button?url=https://www.youtube.com/watch?v={video_id}&fileType=mp3&el=232"
...
# --- Provider 4: MP3Juice ---
async def youtube_to_mp3_mp3juice(video_id: str) -> Dict[str, Optional[str]]:
    try:
        async with httpx.AsyncClient(timeout=20, headers=MP3JUICE_HEADERS) as client:
            ...
            mp3_url = f"https://cdn.mp3j.cc/dl-yt?file={video_id}.mp3&title={quote(f'{title}(mp3j.cc)')}"
...
# --- Orchestrator ---
async def mp3_best_from_videoid(video_id: str) -> Dict[str, Optional[str]]:
    """
    Try all providers in parallel using a YouTube video ID.
    Returns the first successful dowmp3 link.
    """
    tasks = [
        youtube_to_mp3_flvto(video_id),
        youtube_to_mp3_flvto_alt(video_id),
        youtube_to_mp3_flvto_cloudscraper(video_id),  # NEW CLOUDFLARE BYPASS
        youtube_to_mp3_ytmp3(video_id),
        youtube_to_mp3_ytmp3_alt(video_id),
        youtube_to_mp3_insvid(video_id),
        youtube_to_mp3_mp3juice(video_id),
    ]
...
# --- Example Usage ---
'''
video_id = "sUf2PtEZris"  # Example YouTube ID
result = await mp3_best_from_videoid(video_id)
print(result)
'''
